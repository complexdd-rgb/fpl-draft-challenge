(() => {
  ...
  function rebuildRuleRow(row, suppliedCondition = null) {
    ...
    const operatorSet = definition.type === "number" ? NUMBER_OPERATORS : definition.type === "boolean" ? BOOLEAN_OPERATORS : TEXT_OPERATORS;
    ...
    if (definition.type === "boolean") {
      ...
      valueWrap.innerHTML = `<input data-rule-value type="text" value="${escapeAttribute(previousValue)}" placeholder="${definition.type === "manager" ? "e.g. David Moyes" : "e.g. Everton"}" aria-label="Rule text value">`;
    }
  }
  ...
  function validateCondition(condition) {
    ...
    if ((definition.type === "text" || definition.type === "manager") && !String(condition.value).trim()) throw new Error(`${definition.label} needs text.`);
  }
  ...
  function buildLibrarySource() {
    ...
    return `/* FPL Challenge Studio prompt library — exported by Phase 4.
...
`;
  ...
  function compileRuleSource(rule) {
    ...
    return `p => ${expressions.length > 1 ? `(${expressions.join(joiner)})` : expressions[0] || "false"}`;
  }
