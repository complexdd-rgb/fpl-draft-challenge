/* FPL Draft Challenge — retained local stats, share fallback and quick navigation. */
(() => {
  "use strict";
  ...
  function renderExtendedStats() {
    ...
    mount.innerHTML = `<div class="history-chip"><span>Games played</span><strong>${history.length.toLocaleString()}</strong></div><div class="history-chip"><span>Current streak</span><strong>${currentStreak} day${currentStreak === 1 ? "" : "s"}</strong></div><div class="history-chip"><span>Best score</span><strong>${best(scores).toLocaleString()}</strong></div><div class="history-chip"><span>Best efficiency</span><strong>${efficiencies.length ? `${best(efficiencies).toFixed(1)}%` : "0%"}</strong></div><div class="history-chip"><span>Average efficiency</span><strong>${history.length ? `${average(efficiencies).toFixed(1)}%` : "0%"}</strong></div><div class="history-chip"><span>Average penalties</span><strong>${history.length ? average(penalties).toFixed(1) : "0.0"}</strong></div><div class="history-chip"><span>Perfect-pick rate</span><strong>${history.length ? `${perfectPickRate.toFixed(1)}%` : "0%"}</strong></div><div class="history-chip"><span>Exact XI matches</span><strong>${best(exactMatches).toLocaleString()}</strong></div>`;
  }

  // Results v2 replaces these handlers once it loads. Keep this lightweight fallback so
  // sharing still works if the deferred Results module is unavailable.
  ...
  function jumpTarget(target) {
    ...
    if (target === "archive") return document.querySelector(".challenge-calendar-nav") || document.getElementById("grid");
    if (target === "stats") {
  ...
  const results = document.getElementById("results");
  if (results) new MutationObserver(refreshCompletionUi).observe(results, { attributes:true, attributeFilter:["class"] });
  window.addEventListener("fpl:challenge-completed", refreshCompletionUi);
