...
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      body.fpl-visual-overhaul-body .vo-brandbar{justify-content:flex-start;margin-bottom:10px}
      body.fpl-visual-overhaul-body .vo-brandmeta{display:none!important}
      body.fpl-visual-overhaul-body .hero{padding:26px 28px 24px!important}
      ...
      .top-challenge-header{position:relative;z-index:1;display:grid;gap:20px}
      ...
      .top-challenge-title{margin:8px 0 0;max-width:900px;color:#f6fff9;font-size:clamp(1.2rem,3vw,1.8rem);line-height:1.15;letter-spacing:-.035em;font-weight:850;text-wrap:balance}
      .top-challenge-tags{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-top:12px}
      .top-challenge-tag{display:inline-flex;align-items:center;min-height:30px;padding:6px 10px;border:1px solid rgba(255,255,255,.1);border-radius:999px;background:rgba(255,255,255,.035);color:#dbece2;font-size:.61rem;font-weight:900;letter-spacing:.025em}
      .top-challenge-tag.primary{border-color:rgba(0,255,135,.2);background:rgba(0,255,135,.08);color:var(--vo-accent,#00ff87)}

      .top-challenge-stats{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(150px,.7fr) minmax(190px,.9fr);gap:9px;padding-top:2px}
      ...
      @media(max-width:700px){
        body.fpl-visual-overhaul-body .hero{padding:20px 16px 17px!important}
        .top-challenge-header{gap:16px}
    ...
    if (!shell) {
      ...
      shell.innerHTML = `
        <div>
          ...
          <div class="top-challenge-title">${esc(challengeDateLabel(challenge.releaseDate))} · ${esc(challenge.title || "Daily Challenge")}</div>
          <div class="top-challenge-tags">
            <span class="top-challenge-tag primary">${esc(challenge.difficulty || "Daily")}</span>
            <span class="top-challenge-tag">Formation ${esc(challenge.formation || "4-4-2")}</span>
          </div>
        </div>
