/* FPL Draft Challenge — permanent live challenge header. Presentation only. */
(() => {
  "use strict";
  ...
  function addStyles() {
    ...
    style.textContent = `
      main.app > .hero{padding:21px 24px 18px!important}
      main.app > .hero > .eyebrow,
      main.app > .hero > #title,
      main.app > .hero > p,
      main.app > .hero > .v4-meta,
      main.app > .hero > .status,
      main.app > .hero > .v4-progress-shell{display:none!important}

      .top-challenge-header{position:relative;z-index:1;display:grid;gap:14px}
      .top-challenge-kicker{display:flex;align-items:center;gap:8px;color:var(--accent,#00ff87);font-size:.61rem;font-weight:950;letter-spacing:.12em;text-transform:uppercase}
      ...
      .top-challenge-tag.primary{border-color:rgba(0,255,135,.2);background:rgba(0,255,135,.08);color:var(--accent,#00ff87)}
      .top-challenge-stats{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(150px,.7fr) minmax(190px,.9fr);gap:9px}
      ...
      #topHeaderCountdown{font-variant-numeric:tabular-nums;color:#5fe5ff}

      @media(max-width:700px){
        main.app > .hero{padding:18px 15px 15px!important}
  ...
  function install() {
    ...
    document.title = `${challengeDisplayTitle()} · FPL Draft Challenge`;
    return true;
  }
