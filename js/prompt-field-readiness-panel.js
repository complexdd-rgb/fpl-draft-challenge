/* FPL Draft Challenge — Prompt field-readiness panel v1.2.0
...
(() => {
  ...
  function ensureStyle(){if(document.getElementById('promptFieldReadinessStyle'))return;const s=document.createElement('style');s.id='promptFieldReadinessStyle';s.textContent=`
    ...
    #${PANEL_ID} .pfr-actions{display:flex;gap:8px;align-items:center;margin-top:12px;flex-wrap:wrap}
    #${PANEL_ID} .pfr-actions button{border:1px solid rgba(148,163,184,.35);border-radius:8px;padding:6px 9px;background:rgba(148,163,184,.12);color:inherit;cursor:pointer;font-weight:700}
  ...
  function unlockTable(){const result=window.FPL_HISTORICAL_PROMPT_UNLOCK_AUDIT_RESULT;if(!result?.ready)return'';const rows=result.seasons.slice(-12).reverse();return `<div class="pfr-muted" style="margin-top:14px"><strong>Prompt unlock audit</strong> — minimum ${result.minAnswers} distinct answers</div><table class="pfr-season-table"><thead><tr><th>Season</th><th>Unlocked</th><th>GK</th><th>DEF</th><th>MID</th><th>FWD</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${esc(row.season)}</td><td class="pfr-good">${row.unlockedCount}</td><td>${row.byPosition.GK||0}</td><td>${row.byPosition.DEF||0}</td><td>${row.byPosition.MID||0}</td><td>${row.byPosition.FWD||0}</td></tr>`).join('')}</tbody></table>`;}

  function render(){const root=document.querySelector(ROOT);if(!root)return false;ensureStyle();const {meta,prompts,tiers}=summaryData();if(!meta?.ready&&!prompts.some(p=>p.historicalReadiness))return false;let panel=document.getElementById(PANEL_ID);if(!panel){panel=document.createElement('details');panel.id=PANEL_ID;panel.open=false;const anchor=root.querySelector('h1,h2,.workspace-heading,.studio-header')||root.firstElementChild;if(anchor?.parentNode)anchor.parentNode.insertBefore(panel,anchor.nextSibling);else root.prepend(panel);}const tierEntries=Object.entries(tiers).sort((a,b)=>b[1]-a[1]);const fieldEntries=Object.entries(meta?.fieldUsage||{}).slice(0,12);panel.innerHTML=`<summary>Historical prompt readiness <span class="pfr-muted">${prompts.length} prompts mapped</span></summary><div class="pfr-grid">${tierEntries.map(([k,n])=>`<div class="pfr-card"><strong>${n}</strong><span>${esc(label(k))}</span></div>`).join('')}</div>${fieldEntries.length?`<div class="pfr-muted" style="margin-top:10px">Most-used fields: ${fieldEntries.map(([f,n])=>`${esc(f)} (${n})`).join(' · ')}</div>`:''}${seasonTable()}<div class="pfr-actions"><button type="button" data-pfr-run-unlock>Run season prompt unlock audit</button><span class="pfr-muted">Uses real prompt tests; requires 5 distinct answers.</span></div>${unlockTable()}`;const button=panel.querySelector('[data-pfr-run-unlock]');if(button)button.addEventListener('click',()=>{button.disabled=true;button.textContent='Auditing…';setTimeout(()=>{window.FPL_HISTORICAL_PROMPT_UNLOCK_AUDIT?.run?.({minAnswers:5});button.disabled=false;schedule();},0);});decorateCards(prompts);return true;}
  ...
  window.addEventListener('fpl:prompt-field-readiness-ready',schedule);window.addEventListener('fpl:historical-field-manifest-ready',schedule);window.addEventListener('fpl:historical-prompt-unlock-audit-ready',schedule);window.addEventListener('fpl:prompt-library-changed',schedule);window.addEventListener('fpl:prompt-tools-ready',schedule);
  ...
  window.FPL_PROMPT_FIELD_READINESS_PANEL=Object.freeze({version:'1.2.0',render});
})();
