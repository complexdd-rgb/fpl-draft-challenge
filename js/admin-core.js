...
(() => {
  ...
  function renderLiveCard() {
    ...
    elements.liveMeta.textContent = `${formatPublishingDate(liveChallenge.releaseDate)} · ${Number(liveChallenge.perfectScore || 0).toLocaleString()} perfect score`;
  }
  ...
  function buildReport({ prompts, perfect, meta, candidate, history }) {
    ...
    addCheck(checks, !hasDraft ? "pending" : metaDate ? "pass" : "fail", "Date-based challenge identity",
      !hasDraft ? "Generate a draft to check its release date." : metaDate
        ? `${formatPublishingDate(metaDate)} is the challenge identity shown to players; numeric IDs are internal only.`
        : "No release date is set.", true);
    ...
    const dateHistoryMatches = metaDate ? history.filter(entry => entry.releaseDate === metaDate) : [];
    const matchingHistory = dateHistoryMatches.find(entry => historySignature(entry) === candidateSignature);
    const conflictingDateHistory = dateHistoryMatches.some(entry => historySignature(entry) !== candidateSignature);
    const historyState = !hasDraft ? "pending" : conflictingDateHistory ? "fail" : matchingHistory ? "pass" : "fail";
    const historyDetail = !hasDraft ? "Generate and test a draft first." : conflictingDateHistory
      ? `${formatPublishingDate(metaDate)} exists in history with different prompts.`
  ...
  function renderCandidateCard(report) {
    ...
    elements.candidateMeta.textContent = `${formatPublishingDate(report.candidate.releaseDate)} · ${Number(report.candidate.perfectScore || 0).toLocaleString()} perfect score`;
  }
  ...
  function renderComparison(report) {
    ...
    const items = [
      ["Challenge date", live ? formatPublishingDate(live.releaseDate) : "—", candidate ? formatPublishingDate(candidate.releaseDate) : "—"],
  ...
  async function downloadPublishingPack() {
    ...
    try {
      ...
      const dateKey = report.candidate.releaseDate || "next";
      const filename = `fpl-challenge-${dateKey}-publishing-pack.zip`;
  ...
  function buildHistoryMarkdown(history) {
    ...
    for (const entry of sortHistory(history).reverse()) {
      lines.push(`## ${formatPublishingDate(entry.releaseDate)} · ${entry.name || entry.title || "Daily Challenge"}`, "");
      lines.push(`- Release date: ${entry.releaseDate || "—"}`);
      lines.push(`- Difficulty: ${entry.difficulty || "Mixed"}`);
  ...
  function buildReadme(report) {
    return [
      ...
      `Challenge date: ${formatPublishingDate(report.candidate.releaseDate)}`,
  ...
  function formatPublishingDate(value) {
    const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(value || ""));
    if (!match) return String(value || "No date");
    const [, year, month, day] = match;
    return new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "long", year: "numeric", timeZone: "UTC" })
  ...
  function challengeDisplayTitle(challenge) {
    ...
    return challenge.title || `${formatPublishingDate(challenge.releaseDate)} · Daily Challenge`;
  }
  ...
  function sortHistory(history) {
    return [...history].sort((a, b) => String(a.releaseDate || "").localeCompare(String(b.releaseDate || "")));
  }
