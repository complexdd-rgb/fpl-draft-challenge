...
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .player-profile[hidden]{display:none!important}.player-profile-head{display:flex;justify-content:space-between;align-items:flex-start;gap:15px;margin-bottom:15px}.player-profile-head h2{margin:4px 0 4px;font-size:1.35rem;letter-spacing:-.035em}.player-profile-head p{margin:0;color:var(--muted);font-size:.72rem;line-height:1.5}.player-profile-sync{display:inline-flex;align-items:center;gap:6px;flex:0 0 auto;padding:7px 10px;border:1px solid rgba(0,255,135,.16);border-radius:999px;background:rgba(0,255,135,.055);color:var(--accent);font-size:.57rem;font-weight:950;text-transform:uppercase;letter-spacing:.07em}.player-profile-sync::before{content:"";width:6px;height:6px;border-radius:50%;background:currentColor;box-shadow:0 0 10px currentColor}
  ...
  function render(payload) {
    ...
    panel.innerHTML = `
      <div class="player-profile-head">
        <div><span class="overview-kicker">Synced player profile</span><h2>Your verified record</h2><p>Account-wide stats from verified Daily Challenge finishes across every linked device.</p></div>
