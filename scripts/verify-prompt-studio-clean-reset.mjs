...
assert(manifest.manifestVersion === '3.0.0-date-only-daily', 'Central manifest is not on the schedule-manager v2 boundary.');
assert(manifest.assets?.assetManifestRuntime?.version === '3.0.0-date-only-daily', 'Asset-manifest runtime cache version is stale.');
...
assert(generatedManifest.includes('3.0.0-date-only-daily'), 'Generated asset manifest was not refreshed to the schedule-manager v2 boundary.');
