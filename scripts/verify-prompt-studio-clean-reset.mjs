...
assert(manifest.assets?.assetManifestRuntime?.version === '4.0.25-all-family-floor', 'Asset manifest runtime cache tag is stale.');
...
assert(manifest.assets?.adminDailyPublish?.version === '1.1.1-future-publish-boundary', 'Daily publish cache tag is stale.');
assert(manifest.assets?.adminDailyGeneratorGuard?.version === '3.2.3-all-family-floor', 'Daily generator guard cache tag is stale.');
assert(manifest.assets?.adminBatchCalendar?.version === '3.11.0-hard-spacing', 'Batch calendar cache tag is stale.');
assert(manifest.assets?.adminGeneratorProductionCertificationV1?.version === '1.0.1-evidence', 'Generator production certification cache tag is stale.');
