import { adminClient, bodyJson, errorResponse, json, preflight, requireBrowserKey, text } from "../_shared/backend.ts";
...
function publicAllTimeRow(row: AllTimeRow, currentClientId = "") {
  return {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clientId) {
      ...
      if (viewerRow) viewer = publicAllTimeRow(viewerRow as AllTimeRow, clientId);
    }

    return json({
      totalPlayers: Number(count) || 0,
      entries: (data || []).map(row => publicAllTimeRow(row as AllTimeRow, clientId)),
      viewer,
      scoring: {
        dailyMaximum: 100,
